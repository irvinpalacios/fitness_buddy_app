...
(function () {
  ...
  function renderCustomize(state) {
    ...
    const petSetForm = document.getElementById('pet-set-form');
    if (petSetForm) {
      petSetForm.addEventListener('change', function (event) {
        const target = event.target;
        if (!target || target.name !== 'pet-set') return;
        document.dispatchEvent(
          new CustomEvent('changePetSet', {
            detail: { key: target.value },
          })
        );
      });
    }
